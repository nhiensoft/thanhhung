...
const CATEGORIES = ['Tất cả', 'Thiên nhiên UNESCO', 'Văn hóa UNESCO', 'Di tích lịch sử', 'Kiến trúc tôn giáo', 'Danh lam thắng cảnh', 'Kiến trúc hiện đại'];

export const InfiniteHeritageMenu = () => {
  const isMobile = useIsMobile();
  const ITEMS_PER_BATCH = isMobile ? 1 : 4;
  const INITIAL_DISPLAY_COUNT = isMobile ? 1 : 8;
  ...
  // Helper function to check if a site should be visible
  ...
  // Get visible sites count for display (but keep all in DOM)
  const visibleSites = vietnamHeritageSites.filter(shouldShowSite);
  ...
  const handleCardClick = (site: HeritageSite) => {
  ...
  return <section className="py-20 bg-gradient-peaceful min-h-screen">
      {/* Hidden preloader for all heritage images */}
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="text-center mb-12">
          <h2 className="text-4xl md:text-5xl font-playfair font-bold text-foreground mb-4">Vẻ Đẹp Việt Nam</h2>
          <p className="text-lg text-muted-foreground max-w-3xl mx-auto leading-relaxed">
            Khám phá vẻ đẹp di sản văn hóa và thiên nhiên Việt Nam qua hành trình vô tận
          </p>
        </div>
        ...
        <div className="mb-12 space-y-6">
          ...
          <div className="relative max-w-md mx-auto">
            ...
            <Input placeholder="Tìm kiếm di tích, danh lam..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="pl-10 bg-card/80 backdrop-blur-sm border-border/40 focus:border-primary/60" />
          </div>
                    ...
                    <Button
                      ...
                      className={cn(
                        ...
                        selectedRegion === region.id 
                          ? "bg-red-900 text-yellow-300 shadow-[0_0_15px_rgba(251,191,36,0.5)] border-yellow-400 scale-110 border" 
                          : "hover:bg-primary hover:text-primary-foreground hover:shadow-lg hover:scale-105 text-foreground border border-transparent"
                      )}
                    >
          ...
          <div className="flex flex-wrap justify-center gap-2 max-w-4xl mx-auto">
            {CATEGORIES.map(category => <Badge key={category} variant={selectedCategory === category ? "default" : "outline"} className={cn("cursor-pointer transition-all duration-300 hover:scale-105", selectedCategory === category ? "bg-primary text-primary-foreground shadow-lg" : "hover:bg-primary/10 border-border/40")} onClick={() => setSelectedCategory(category)}>
                {category}
              </Badge>)}
          </div>
        </div>
        ...
        <div className="text-center mb-8">
          <p className="text-muted-foreground">
            Hiển thị <span className="font-semibold text-primary">{Math.min(displayCount, visibleSites.length)}</span> 
            {" "}trên <span className="font-semibold text-primary">{visibleSites.length}</span> di tích
          </p>
        </div>
        ...
        <div className="relative min-h-[600px]">
          ...
          <div className="absolute inset-0 overflow-hidden pointer-events-none">
            <div className="absolute w-full h-full">
              {Array.from({
              length: 8
            }).map((_, i) => <div key={i} className={cn("absolute w-2 h-2 bg-primary/20 rounded-full", i % 2 === 0 ? "animate-drift" : "animate-drift-reverse")} style={{
              top: `${Math.random() * 100}%`,
          ...
          {/* Heritage Cards Grid - Always render all cards, control visibility with CSS */}
              ...
              {vietnamHeritageSites.map((site, index) => {
                ...
                return <div 
                  ...
                  className={cn(
                    ...
                    shouldDisplay ? 'block animate-fade-in' : 'hidden'
                  )}
                  style={{ animationDelay: shouldDisplay ? `${(visibleIndex >= 0 ? visibleIndex % 8 : 0) * 0.1}s` : undefined }}
                >
                  <HeritageFloatingCard site={site} index={index} onClick={() => handleCardClick(site)} />
                </div>
              })}
            </div>
            ...
            {visibleSites.length === 0 && (
              <div className="text-center py-20">
                <p className="text-muted-foreground text-lg">
                  Không tìm thấy nội dung phù hợp với mong muốn của bạn trên website
                </p>
              </div>
            )}
          </div>
          ...
          {/* Load More Button */}
          {hasMore && visibleSites.length > 0 && <div className="text-center mt-12">
              <Button onClick={loadMore} variant="outline" size="lg" className="bg-card/80 backdrop-blur-sm border-border/40 hover:bg-primary/10 hover:border-primary/40 transition-all duration-300">
